...
(->
  ...
  navbar_collapsed_bars = "<a class=\"btn btn-navbar\" data-toggle=\"collapse\" data-target=\".nav-collapse\">" + "  <span class=\"icon-bar\"></span>" + "  <span class=\"icon-bar\"></span>" + "  <span class=\"icon-bar\"></span>" + "</a>"
  ...
  year_changelog.find("select[name=newyear] option").each (index) ->
    # Ignore initial option/helptext
    year_dropdown_list.append "<li><a href=\"" + $(this).attr("value") + "\">" + $(this).html() + "</a></li>"  if index > 0
  ...
  field_titles = title_row.find("th").slice(1).map((i, val) ->
    $(val).html()
  )
  value_row = main_cate_table.find("td table td table tr:eq(1)")
  field_values = value_row.find("td big b").map((i, val) ->
    $(val).html().replace "<br>", " "
  )
  ...
  field_titles.each (index) ->
    identity_table.append "<tr><td><b>" + field_titles[index] + "</b></td><td>" + field_values[index] + "</td></tr>"  if index > 0
  ...
  form = $("<form class=\"form-horizontal\" action=\"/timetable.cgi\" type=\"GET\" >" + "    <div class=\"control-group\">" + "        <label class=\"control-label\" for=\"period-select\">Period</label>" + "        <div id=\"period-select-container\" class=\"controls\">" + "        </div>" + "    </div>" + "    <div class=\"control-group\">" + "        <label class=\"control-label\" for=\"class-select\">Class</label>" + "        <div id=\"class-select-container\" class=\"controls\">" + "        </div>" + "    </div>" + "    <div class=\"control-group\">" + "        <div id=\"button-container\" class=\"controls\">" + "        </div>" + "    </div>" + "</form>")
